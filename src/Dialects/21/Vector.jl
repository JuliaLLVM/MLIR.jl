...
```
"""
function deinterleave(source::Value; res1=nothing::Union{Nothing, IR.Type}, res2=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[source, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(res1) && push!(op_ty_results, res1)
    !isnothing(res2) && push!(op_ty_results, res2)
    
    IR.create_operation(
        "vector.deinterleave", location;
        operands, owned_regions, successors, attributes,
        results=(length(op_ty_results) == 0 ? nothing : op_ty_results),
        result_inference=(length(op_ty_results) == 0 ? true : false)
    )
...
```
"""
function from_elements(elements::Vector{Value}; dest::IR.Type, location=Location())
...
```
"""
function insert(valueToStore::Value, dest::Value, dynamic_position::Vector{Value}; result=nothing::Union{Nothing, IR.Type}, static_position, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[valueToStore, dest, dynamic_position..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("static_position", static_position), ]
    !isnothing(result) && push!(op_ty_results, result)
...
```
"""
function insert_strided_slice(valueToStore::Value, dest::Value; result=nothing::Union{Nothing, IR.Type}, offsets, strides, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[valueToStore, dest, ]
    owned_regions = Region[]
    successors = Block[]
...
function multi_reduction(source::Value, acc::Value; dest=nothing::Union{Nothing, IR.Type}, kind, reduction_dims, location=Location())
    ...
    !isnothing(dest) && push!(op_ty_results, dest)
...
```
"""
function scalable_extract(source::Value; result::IR.Type, pos, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[source, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function scalable_insert(valueToStore::Value, dest::Value; result=nothing::Union{Nothing, IR.Type}, pos, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[valueToStore, dest, ]
    owned_regions = Region[]
    successors = Block[]
...
function scan(source::Value, initial_value::Value; dest=nothing::Union{Nothing, IR.Type}, accumulated_value=nothing::Union{Nothing, IR.Type}, kind, reduction_dim, inclusive, location=Location())
    ...
    !isnothing(dest) && push!(op_ty_results, dest)
    !isnothing(accumulated_value) && push!(op_ty_results, accumulated_value)
...
function shuffle(v1::Value, v2::Value; vector=nothing::Union{Nothing, IR.Type}, mask, location=Location())
    ...
    !isnothing(vector) && push!(op_ty_results, vector)
...
```
"""
function step(; result::IR.Type, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function to_elements(source::Value; elements::Vector{IR.Type}, location=Location())
    op_ty_results = IR.Type[elements..., ]
    operands = Value[source, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
```
"""
function transfer_read(base::Value, indices::Vector{Value}, padding::Value, mask=nothing::Union{Nothing, Value}; vector::IR.Type, permutation_map, in_bounds, location=Location())
    op_ty_results = IR.Type[vector, ]
    operands = Value[base, indices..., padding, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("permutation_map", permutation_map), NamedAttribute("in_bounds", in_bounds), ]
    !isnothing(mask) && push!(operands, mask)
    push!(attributes, operandsegmentsizes([1, length(indices), 1, Int(!isnothing(mask)), ]))
...
```
"""
function transfer_write(valueToStore::Value, base::Value, indices::Vector{Value}, mask=nothing::Union{Nothing, Value}; result=nothing::Union{Nothing, IR.Type}, permutation_map, in_bounds, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[valueToStore, base, indices..., ]
    owned_regions = Region[]
    successors = Block[]
