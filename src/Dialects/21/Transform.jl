...
"""
`affine_simplify_min_max_affine_ops`
...
"""
function affine_simplify_min_max_affine_ops(target::Value; location=Location())
    op_ty_results = IR.Type[]
    ...
    IR.create_operation(
        "transform.affine.simplify_min_max_affine_ops", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function bufferization_empty_tensor_to_alloc_tensor(target::Value; transformed::IR.Type, location=Location())
    op_ty_results = IR.Type[transformed, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
function bufferization_one_shot_bufferize(target::Value; transformed::IR.Type, function_boundary_type_conversion=nothing, allow_return_allocs_from_loops=nothing, allow_unknown_ops=nothing, bufferize_function_boundaries=nothing, dump_alias_sets=nothing, test_analysis_only=nothing, print_conflicts=nothing, check_parallel_regions=nothing, memcpy_op=nothing, location=Location())
    ...
    !isnothing(check_parallel_regions) && push!(attributes, NamedAttribute("check_parallel_regions", check_parallel_regions))
...
function apply_conversion_patterns_func_func_to_llvm(; location=Location())
    ...
    IR.create_operation(
        "transform.apply_conversion_patterns.func.func_to_llvm", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
the output types.
"""
function func_cast_and_call(insertion_point::Value, inputs=nothing::Union{Nothing, Value}; outputs=nothing::Union{Nothing, Value}, function_=nothing::Union{Nothing, Value}, result::IR.Type, insert_after=nothing, function_name=nothing, conversions::Region, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[insertion_point, ]
    owned_regions = Region[conversions, ]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(inputs) && push!(operands, inputs)
    !isnothing(outputs) && push!(operands, outputs)
    !isnothing(function_) && push!(operands, function_)
    push!(attributes, operandsegmentsizes([1, Int(!isnothing(inputs)), Int(!isnothing(outputs)), Int(!isnothing(function_)), ]))
    !isnothing(insert_after) && push!(attributes, NamedAttribute("insert_after", insert_after))
    !isnothing(function_name) && push!(attributes, NamedAttribute("function_name", function_name))
    
    IR.create_operation(
        "transform.func.cast_and_call", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function func_replace_func_signature(module_::Value; transformed_module::IR.Type, transformed_function::IR.Type, function_name, args_interchange, results_interchange, adjust_func_calls=nothing, location=Location())
    op_ty_results = IR.Type[transformed_module, transformed_function, ]
    operands = Value[module_, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("function_name", function_name), NamedAttribute("args_interchange", args_interchange), NamedAttribute("results_interchange", results_interchange), ]
    !isnothing(adjust_func_calls) && push!(attributes, NamedAttribute("adjust_func_calls", adjust_func_calls))
    
    IR.create_operation(
        "transform.func.replace_func_signature", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
`apply_patterns_gpu_gpu_shuffle_to_amdgpu`

Collects patterns that are tryin to promote `gpu.shuffle`s to specialized
AMDGPU intrinsics.
"""
function apply_patterns_gpu_gpu_shuffle_to_amdgpu(; location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.apply_patterns.gpu.gpu_shuffle_to_amdgpu", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function apply_conversion_patterns_gpu_gpu_to_nvvm(; benefit=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(benefit) && push!(attributes, NamedAttribute("benefit", benefit))
...
"""
`apply_conversion_patterns_gpu_gpu_to_rocdl`

Collects patterns that convert GPU dialect ops to ROCDL dialect ops. These
patterns require an \"LLVMTypeConverter\".
"""
function apply_conversion_patterns_gpu_gpu_to_rocdl(; chipset, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("chipset", chipset), ]
    
    IR.create_operation(
        "transform.apply_conversion_patterns.gpu.gpu_to_rocdl", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function match_structured_classify_contraction_dims(operand_handle::Value; batch::IR.Type, m::IR.Type, n::IR.Type, k::IR.Type, location=Location())
    op_ty_results = IR.Type[batch, m, n, k, ]
    operands = Value[operand_handle, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
function match_structured_classify_convolution_dims(operand_handle::Value; batch::IR.Type, output_image::IR.Type, output_channel::IR.Type, filter_loop::IR.Type, input_channel::IR.Type, depth::IR.Type, strides::IR.Type, dilations::IR.Type, location=Location())
    op_ty_results = IR.Type[batch, output_image, output_channel, filter_loop, input_channel, depth, strides, dilations, ]
    operands = Value[operand_handle, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
"""
function apply_patterns_linalg_decompose_pack_unpack(; location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.apply_patterns.linalg.decompose_pack_unpack", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function apply_patterns_linalg_decompose_pad(; location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.apply_patterns.linalg.decompose_pad", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function apply_patterns_linalg_fold_add_into_dest(; location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.apply_patterns.linalg.fold_add_into_dest", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
// TODO:
"""
function apply_patterns_linalg_fold_pack_unpack_into_empty(; fold_single_use_only=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.apply_patterns.linalg.fold_pack_unpack_into_empty", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function apply_patterns_linalg_pad_vectorization(; location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.apply_patterns.linalg.pad_vectorization", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function structured_bufferize_to_allocation(target::Value; allocated_buffer::IR.Type, new_ops::IR.Type, memory_space=nothing, memcpy_op=nothing, alloc_op=nothing, bufferize_destination_only=nothing, emit_dealloc=nothing, location=Location())
    op_ty_results = IR.Type[allocated_buffer, new_ops, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
"""
function structured_continuous_tile_sizes(target::Value; tile_sizes::IR.Type, chunk_sizes::IR.Type, dimension, target_size, location=Location())
    op_ty_results = IR.Type[tile_sizes, chunk_sizes, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.structured.continuous_tile_sizes", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function structured_convert_conv2d_to_img2col(target::Value; img2col_tensor::IR.Type, transformed::IR.Type, location=Location())
    op_ty_results = IR.Type[img2col_tensor, transformed, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
"""
`structured_decompose_winograd_op`
...
operations.
"""
function structured_decompose_winograd_op(target::Value; transformed::IR.Type, location=Location())
    op_ty_results = IR.Type[transformed, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.structured.decompose_winograd_op", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function structured_fuse_into_containing_op(producer_op::Value, containing_op::Value; fused_op::IR.Type, new_containing_op::IR.Type, location=Location())
    op_ty_results = IR.Type[fused_op, new_containing_op, ]
...
fusion steps.
"""
function structured_fuse(target::Value; transformed::IR.Type, loops::Vector{IR.Type}, tile_sizes=nothing, tile_interchange=nothing, apply_cleanup=nothing, location=Location())
    op_ty_results = IR.Type[transformed, loops..., ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    ...
    !isnothing(apply_cleanup) && push!(attributes, NamedAttribute("apply_cleanup", apply_cleanup))
...
function structured_hoist_pad_build_packing_loop_nest(target::Value, loop::Value; packing_loop::IR.Type, transpose=nothing, location=Location())
    op_ty_results = IR.Type[packing_loop, ]
...
function op.
"""
function structured_hoist_redundant_vector_broadcasts(target::Value; transformed::IR.Type, location=Location())
    op_ty_results = IR.Type[transformed, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.structured.hoist_redundant_vector_broadcasts", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function op.
"""
function structured_hoist_redundant_vector_transfers(target::Value; transformed::IR.Type, verify_non_zero_trip=nothing, location=Location())
    op_ty_results = IR.Type[transformed, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(verify_non_zero_trip) && push!(attributes, NamedAttribute("verify_non_zero_trip", verify_non_zero_trip))
...
"""
`structured_linalg_copy_to_memref`
...
"""
function structured_linalg_copy_to_memref(target::Value; transformed::IR.Type, location=Location())
    op_ty_results = IR.Type[transformed, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.structured.linalg_copy_to_memref", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function structured_lower_pack(target::Value; pad_op::IR.Type, expand_shape_op::IR.Type, transpose_op::IR.Type, lowerPadLikeWithInsertSlice=nothing, location=Location())
    op_ty_results = IR.Type[pad_op, expand_shape_op, transpose_op, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(lowerPadLikeWithInsertSlice) && push!(attributes, NamedAttribute("lowerPadLikeWithInsertSlice", lowerPadLikeWithInsertSlice))
...
"""
function structured_lower_unpack(target::Value; empty_op::IR.Type, transpose_op::IR.Type, collapse_shape_op::IR.Type, extract_slice_op::IR.Type, lowerUnpadLikeWithExtractSlice=nothing, location=Location())
    op_ty_results = IR.Type[empty_op, transpose_op, collapse_shape_op, extract_slice_op, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(lowerUnpadLikeWithExtractSlice) && push!(attributes, NamedAttribute("lowerUnpadLikeWithExtractSlice", lowerUnpadLikeWithExtractSlice))
...
function structured_gpu_map_copy_to_threads(target::Value; forall_op::IR.Type, tiled_op::IR.Type, total_num_threads, desired_bit_alignment, location=Location())
    op_ty_results = IR.Type[forall_op, tiled_op, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
...
function structured_multitile_sizes(target::Value; low_size::IR.Type, high_size::IR.Type, split_point::IR.Type, dimension, target_size, divisor=nothing, location=Location())
    op_ty_results = IR.Type[low_size, high_size, split_point, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("dimension", dimension), NamedAttribute("target_size", target_size), ]
...
function structured_pack_greedily(target::Value, matmul_packed_sizes::Vector{Value}; packed_op::IR.Type, static_matmul_packed_sizes=nothing, matmul_padded_sizes_next_multiple_of=nothing, matmul_inner_dims_order=nothing, location=Location())
    op_ty_results = IR.Type[packed_op, ]
...
function structured_pack_transpose(target_pack_or_un_pack_op::Value, target_linalg_op::Value; packed_op::IR.Type, pack_op::IR.Type, un_pack_op::IR.Type, outer_perm=nothing, inner_perm=nothing, location=Location())
    op_ty_results = IR.Type[packed_op, pack_op, un_pack_op, ]
...
"""
function structured_pad(target::Value, pad_to_multiple_of::Vector{Value}; padded::IR.Type, pad::IR.Type, copy::IR.Type, padding_values=nothing, padding_dimensions=nothing, static_pad_to_multiple_of=nothing, nofold_flags=nothing, transpose_paddings=nothing, copy_back_op=nothing, use_prescribed_tensor_shapes=nothing, location=Location())
    op_ty_results = IR.Type[padded, pad, copy, ]
    operands = Value[target, pad_to_multiple_of..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    ...
    !isnothing(static_pad_to_multiple_of) && push!(attributes, NamedAttribute("static_pad_to_multiple_of", static_pad_to_multiple_of))
    !isnothing(nofold_flags) && push!(attributes, NamedAttribute("nofold_flags", nofold_flags))
    ...
    !isnothing(use_prescribed_tensor_shapes) && push!(attributes, NamedAttribute("use_prescribed_tensor_shapes", use_prescribed_tensor_shapes))
...
"""
`structured_pad_tiling_interface`
...
padded operations, which can be empty.
"""
function structured_pad_tiling_interface(target::Value, padding_sizes::Vector{Value}; padded::IR.Type, pad::IR.Type, padding_values=nothing, static_padding_sizes=nothing, pad_to_multiple_of=nothing, location=Location())
    op_ty_results = IR.Type[padded, pad, ]
    operands = Value[target, padding_sizes..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(padding_values) && push!(attributes, NamedAttribute("padding_values", padding_values))
    !isnothing(static_padding_sizes) && push!(attributes, NamedAttribute("static_padding_sizes", static_padding_sizes))
    !isnothing(pad_to_multiple_of) && push!(attributes, NamedAttribute("pad_to_multiple_of", pad_to_multiple_of))
    
    IR.create_operation(
        "transform.structured.pad_tiling_interface", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function structured_promote(target::Value; transformed::IR.Type, operands_to_promote=nothing, use_full_tile_buffers=nothing, use_full_tiles_by_default=nothing, use_original_subview_size=nothing, use_alloca=nothing, memory_space=nothing, mapping=nothing, alignment=nothing, location=Location())
    op_ty_results = IR.Type[transformed, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    ...
    !isnothing(use_original_subview_size) && push!(attributes, NamedAttribute("use_original_subview_size", use_original_subview_size))
...
function structured_replace(target::Value; replacement::IR.Type, bodyRegion::Region, location=Location())
    op_ty_results = IR.Type[replacement, ]
    operands = Value[target, ]
...
"""
function structured_split(target::Value, dynamic_chunk_sizes=nothing::Union{Nothing, Value}; split_list::IR.Type, dimension, static_chunk_sizes, multiway=nothing, location=Location())
    op_ty_results = IR.Type[split_list, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("dimension", dimension), NamedAttribute("static_chunk_sizes", static_chunk_sizes), ]
    !isnothing(dynamic_chunk_sizes) && push!(operands, dynamic_chunk_sizes)
    !isnothing(multiway) && push!(attributes, NamedAttribute("multiway", multiway))
...
function structured_split_reduction(target::Value; init_or_alloc_op::IR.Type, fill_op::IR.Type, split_linalg_op::IR.Type, combining_linalg_op::IR.Type, split_factor=nothing, insert_split_dimension=nothing, inner_parallel=nothing, use_scaling_algorithm=nothing, use_alloc=nothing, location=Location())
    op_ty_results = IR.Type[init_or_alloc_op, fill_op, split_linalg_op, combining_linalg_op, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
```
"""
function structured_tile_reduction_using_for(target::Value; fill_op::Vector{IR.Type}, split_op::IR.Type, combining_op::IR.Type, for_op::IR.Type, reduction_dims=nothing, tile_sizes=nothing, location=Location())
    op_ty_results = IR.Type[fill_op..., split_op, combining_op, for_op, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(reduction_dims) && push!(attributes, NamedAttribute("reduction_dims", reduction_dims))
    !isnothing(tile_sizes) && push!(attributes, NamedAttribute("tile_sizes", tile_sizes))
...
```
"""
function structured_tile_reduction_using_forall(target::Value; fill_op::Vector{IR.Type}, split_linalg_op::IR.Type, combining_linalg_op::IR.Type, forall_op::IR.Type, reduction_dims=nothing, num_threads=nothing, tile_sizes=nothing, mapping=nothing, location=Location())
    op_ty_results = IR.Type[fill_op..., split_linalg_op, combining_linalg_op, forall_op, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
function structured_tile_using_for(target::Value, dynamic_sizes::Vector{Value}; tiled_linalg_op::IR.Type, loops::Vector{IR.Type}, static_sizes=nothing, interchange=nothing, scalable_sizes=nothing, location=Location())
    op_ty_results = IR.Type[tiled_linalg_op, loops..., ]
...
function structured_tile_using_forall(target::Value, num_threads::Vector{Value}, tile_sizes::Vector{Value}, packed_num_threads=nothing::Union{Nothing, Value}; packed_tile_sizes=nothing::Union{Nothing, Value}, tiled_op::IR.Type, forall_op::IR.Type, static_num_threads=nothing, static_tile_sizes=nothing, mapping=nothing, location=Location())
    op_ty_results = IR.Type[tiled_op, forall_op, ]
    ...
    push!(attributes, operandsegmentsizes([1, length(num_threads), length(tile_sizes), Int(!isnothing(packed_num_threads)), Int(!isnothing(packed_tile_sizes)), ]))
...
"""
function structured_transpose_matmul(target::Value; transformed::IR.Type, inputToTranspose=nothing, location=Location())
    op_ty_results = IR.Type[transformed, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(inputToTranspose) && push!(attributes, NamedAttribute("inputToTranspose", inputToTranspose))
    
    IR.create_operation(
        "transform.structured.transpose_matmul", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
mentioned above.
"""
function structured_vectorize(target::Value, vector_sizes::Vector{Value}; static_vector_sizes=nothing, vectorize_nd_extract=nothing, scalable_sizes=nothing, location=Location())
    op_ty_results = IR.Type[]
...
"""
function structured_winograd_conv2d(target::Value; transformed::IR.Type, fmr, location=Location())
    op_ty_results = IR.Type[transformed, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("fmr", fmr), ]
    
    IR.create_operation(
        "transform.structured.winograd_conv2d", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function memref_alloca_to_global(alloca::Value; getGlobal::IR.Type, global_::IR.Type, location=Location())
    op_ty_results = IR.Type[getGlobal, global_, ]
...
function apply_conversion_patterns_scf_scf_to_control_flow(; location=Location())
    ...
    IR.create_operation(
        "transform.apply_conversion_patterns.scf.scf_to_control_flow", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
requested.
"""
function loop_forall_to_parallel(target::Value; transformed::Vector{IR.Type}, location=Location())
    op_ty_results = IR.Type[transformed..., ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.loop.forall_to_parallel", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function loop_fuse_sibling(target::Value, source::Value; fused_loop::IR.Type, location=Location())
    op_ty_results = IR.Type[fused_loop, ]
...
function loop_outline(target::Value; function_::IR.Type, call::IR.Type, func_name, location=Location())
    op_ty_results = IR.Type[function_, call, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
...
function loop_peel(target::Value; peeled_loop::IR.Type, remainder_loop::IR.Type, peel_front=nothing, fail_if_already_divisible=nothing, location=Location())
    op_ty_results = IR.Type[peeled_loop, remainder_loop, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
function loop_unroll_and_jam(target::Value; factor, location=Location())
    ...
    IR.create_operation(
        "transform.loop.unroll_and_jam", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function sparse_tensor_match_sparse_inout(target::Value; result::IR.Type, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.sparse_tensor.match.sparse_inout", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
`apply_patterns_tensor_bubble_up_extract_slice`

Indicates that producers of tensor.extract_slice should swap and operate on 
the result of the slice.
"""
function apply_patterns_tensor_bubble_up_extract_slice(; location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.apply_patterns.tensor.bubble_up_extract_slice", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function apply_patterns_tensor_rewrite_as_constant(; aggressive=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(aggressive) && push!(attributes, NamedAttribute("aggressive", aggressive))
...
"""
function apply_patterns(target::Value; apply_cse=nothing, max_iterations=nothing, max_num_rewrites=nothing, patterns::Region, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[target, ]
    ...
    !isnothing(max_iterations) && push!(attributes, NamedAttribute("max_iterations", max_iterations))
    !isnothing(max_num_rewrites) && push!(attributes, NamedAttribute("max_num_rewrites", max_num_rewrites))
...
of targeted ops.
"""
function apply_registered_pass(target::Value, dynamic_options::Vector{Value}; result::IR.Type, pass_name, options=nothing, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[target, dynamic_options..., ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function foreach_match(root::Value, forwarded_inputs::Vector{Value}; updated::IR.Type, forwarded_outputs::Vector{IR.Type}, restrict_root=nothing, flatten_results=nothing, matchers, actions, location=Location())
    op_ty_results = IR.Type[updated, forwarded_outputs..., ]
    operands = Value[root, forwarded_inputs..., ]
    owned_regions = Region[]
    successors = Block[]
    ...
    !isnothing(flatten_results) && push!(attributes, NamedAttribute("flatten_results", flatten_results))
...
"""
function foreach(targets::Vector{Value}; results::Vector{IR.Type}, with_zip_shortest=nothing, body::Region, location=Location())
    op_ty_results = IR.Type[results..., ]
    operands = Value[targets..., ]
    owned_regions = Region[body, ]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(with_zip_shortest) && push!(attributes, NamedAttribute("with_zip_shortest", with_zip_shortest))
...
function get_consumers_of_result(target::Value; consumers::IR.Type, result_number, location=Location())
    op_ty_results = IR.Type[consumers, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
...
function get_producer_of_operand(target::Value; producer::IR.Type, operand_number, location=Location())
    op_ty_results = IR.Type[producer, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
...
function get_type(value::Value; type_param::IR.Type, elemental=nothing, location=Location())
    op_ty_results = IR.Type[type_param, ]
...
"""
function include_(operands::Vector{Value}; results::Vector{IR.Type}, target, failure_propagation_mode, arg_attrs=nothing, res_attrs=nothing, location=Location())
    op_ty_results = IR.Type[results..., ]
...
"""
function print(target=nothing::Union{Nothing, Value}; name=nothing, assume_verified=nothing, use_local_scope=nothing, skip_regions=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    ...
    !isnothing(assume_verified) && push!(attributes, NamedAttribute("assume_verified", assume_verified))
    !isnothing(use_local_scope) && push!(attributes, NamedAttribute("use_local_scope", use_local_scope))
    !isnothing(skip_regions) && push!(attributes, NamedAttribute("skip_regions", skip_regions))
...
function replicate(pattern::Value, handles::Vector{Value}; replicated::Vector{IR.Type}, location=Location())
    op_ty_results = IR.Type[replicated..., ]
...
function sequence(root=nothing::Union{Nothing, Value}; extra_bindings::Vector{Value}, results::Vector{IR.Type}, failure_propagation_mode, body::Region, location=Location())
    ...
    push!(attributes, operandsegmentsizes([Int(!isnothing(root)), length(extra_bindings), ]))
...
"""
function debug_emit_param_as_remark(param::Value, anchor=nothing::Union{Nothing, Value}; message=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[param, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(anchor) && push!(operands, anchor)
    !isnothing(message) && push!(attributes, NamedAttribute("message", message))
    
    IR.create_operation(
        "transform.debug.emit_param_as_remark", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function debug_emit_remark_at(at::Value; message, location=Location())
    ...
    IR.create_operation(
        "transform.debug.emit_remark_at", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function loop_hoist_loop_invariant_subsets(target::Value; location=Location())
    ...
    IR.create_operation(
        "transform.loop.hoist_loop_invariant_subsets", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function apply_patterns_vector_drop_unit_dims_with_shape_cast(; location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.apply_patterns.vector.drop_unit_dims_with_shape_cast", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function apply_patterns_vector_elementwise_to_vector(; location=Location())
    ...
    IR.create_operation(
        "transform.apply_patterns.vector.elementwise_to_vector", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function apply_patterns_vector_interleave_to_shuffle(; location=Location())
    ...
    IR.create_operation(
        "transform.apply_patterns.vector.interleave_to_shuffle", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
function apply_patterns_vector_lower_bitcast(; location=Location())
    ...
    IR.create_operation(
        "transform.apply_patterns.vector.lower_bitcast", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
`apply_patterns_vector_sink_mem_ops`
...
```
"""
function apply_patterns_vector_sink_mem_ops(; location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.apply_patterns.vector.sink_mem_ops", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
`apply_patterns_vector_sink_ops`
...
"""
function apply_patterns_vector_sink_ops(; location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    ...
    IR.create_operation(
        "transform.apply_patterns.vector.sink_ops", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function apply_conversion_patterns_vector_vector_to_llvm(; reassociate_fp_reductions=nothing, force_32bit_vector_indices=nothing, use_vector_alignment=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    ...
    !isnothing(use_vector_alignment) && push!(attributes, NamedAttribute("use_vector_alignment", use_vector_alignment))
