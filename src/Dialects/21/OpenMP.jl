...
```
"""
function canonical_loop(tripCount::Value, cli=nothing::Union{Nothing, Value}; region::Region, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[tripCount, ]
    owned_regions = Region[region, ]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(cli) && push!(operands, cli)
    
    create_operation(
        "omp.canonical_loop", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function declare_mapper_info(map_vars::Vector{Value}; location=Location())
    op_ty_results = IR.Type[]
    operands = Value[map_vars..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    
    create_operation(
        "omp.declare_mapper.info", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function declare_mapper(; sym_name, type, body::Region, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
...
"""
function distribute(allocate_vars::Vector{Value}, allocator_vars::Vector{Value}, dist_schedule_chunk_size=nothing::Union{Nothing, Value}; private_vars::Vector{Value}, dist_schedule_static=nothing, order=nothing, order_mod=nothing, private_syms=nothing, private_needs_barrier=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[]
    ...
    !isnothing(private_needs_barrier) && push!(attributes, NamedAttribute("private_needs_barrier", private_needs_barrier))
...
"""
function loop(private_vars::Vector{Value}, reduction_vars::Vector{Value}; bind_kind=nothing, private_syms=nothing, private_needs_barrier=nothing, order=nothing, order_mod=nothing, reduction_mod=nothing, reduction_byref=nothing, reduction_syms=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[]
...
"""
function map_info(var_ptr::Value, var_ptr_ptr=nothing::Union{Nothing, Value}; members::Vector{Value}, bounds::Vector{Value}, omp_ptr::IR.Type, var_type, map_type, map_capture_type, members_index=nothing, mapper_id=nothing, name=nothing, partial_map=nothing, location=Location())
    op_ty_results = IR.Type[omp_ptr, ]
    operands = Value[var_ptr, members..., bounds..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("var_type", var_type), NamedAttribute("map_type", map_type), NamedAttribute("map_capture_type", map_capture_type), ]
    ...
    !isnothing(mapper_id) && push!(attributes, NamedAttribute("mapper_id", mapper_id))
    !isnothing(name) && push!(attributes, NamedAttribute("name", name))
...
"""
function new_cli(; result::IR.Type, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
...
which is available in the accumulator after they all complete.
"""
function parallel(allocate_vars::Vector{Value}, allocator_vars::Vector{Value}, if_expr=nothing::Union{Nothing, Value}; num_threads=nothing::Union{Nothing, Value}, private_vars::Vector{Value}, reduction_vars::Vector{Value}, private_syms=nothing, private_needs_barrier=nothing, proc_bind_kind=nothing, reduction_mod=nothing, reduction_byref=nothing, reduction_syms=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[]
...
"""
function private(; sym_name, type, data_sharing_type, init_region::Region, copy_region::Region, dealloc_region::Region, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[init_region, copy_region, dealloc_region, ]
    successors = Block[]
...
which is available in the accumulator after they all complete.
"""
function sections(allocate_vars::Vector{Value}, allocator_vars::Vector{Value}, private_vars::Vector{Value}, reduction_vars::Vector{Value}; nowait=nothing, private_syms=nothing, private_needs_barrier=nothing, reduction_mod=nothing, reduction_byref=nothing, reduction_syms=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[allocate_vars..., allocator_vars..., private_vars..., reduction_vars..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
"""
function simd(aligned_vars::Vector{Value}, if_expr=nothing::Union{Nothing, Value}; linear_vars::Vector{Value}, linear_step_vars::Vector{Value}, nontemporal_vars::Vector{Value}, private_vars::Vector{Value}, reduction_vars::Vector{Value}, alignments=nothing, order=nothing, order_mod=nothing, private_syms=nothing, private_needs_barrier=nothing, reduction_mod=nothing, reduction_byref=nothing, reduction_syms=nothing, safelen=nothing, simdlen=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[]
...
progress even if the child operation has not completed yet.
"""
function single(allocate_vars::Vector{Value}, allocator_vars::Vector{Value}, copyprivate_vars::Vector{Value}, private_vars::Vector{Value}; copyprivate_syms=nothing, nowait=nothing, private_syms=nothing, private_needs_barrier=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[]
...
"""
function taskloop(allocate_vars::Vector{Value}, allocator_vars::Vector{Value}, final=nothing::Union{Nothing, Value}; grainsize=nothing::Union{Nothing, Value}, if_expr=nothing::Union{Nothing, Value}, in_reduction_vars::Vector{Value}, num_tasks=nothing::Union{Nothing, Value}, priority=nothing::Union{Nothing, Value}, private_vars::Vector{Value}, reduction_vars::Vector{Value}, grainsize_mod=nothing, in_reduction_byref=nothing, in_reduction_syms=nothing, mergeable=nothing, nogroup=nothing, num_tasks_mod=nothing, private_syms=nothing, private_needs_barrier=nothing, reduction_mod=nothing, reduction_byref=nothing, reduction_syms=nothing, untied=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[]
    ...
    !isnothing(grainsize_mod) && push!(attributes, NamedAttribute("grainsize_mod", grainsize_mod))
    !isnothing(in_reduction_byref) && push!(attributes, NamedAttribute("in_reduction_byref", in_reduction_byref))
    !isnothing(in_reduction_syms) && push!(attributes, NamedAttribute("in_reduction_syms", in_reduction_syms))
    !isnothing(mergeable) && push!(attributes, NamedAttribute("mergeable", mergeable))
    !isnothing(nogroup) && push!(attributes, NamedAttribute("nogroup", nogroup))
    !isnothing(num_tasks_mod) && push!(attributes, NamedAttribute("num_tasks_mod", num_tasks_mod))
    !isnothing(private_syms) && push!(attributes, NamedAttribute("private_syms", private_syms))
    !isnothing(private_needs_barrier) && push!(attributes, NamedAttribute("private_needs_barrier", private_needs_barrier))
    !isnothing(reduction_mod) && push!(attributes, NamedAttribute("reduction_mod", reduction_mod))
    !isnothing(reduction_byref) && push!(attributes, NamedAttribute("reduction_byref", reduction_byref))
...
Assembly formats:
  ...
  omp.unroll_heuristic(%cli) -> ()
"""
function unroll_heuristic(applyee::Value; location=Location())
    op_ty_results = IR.Type[]
    operands = Value[applyee, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    
    create_operation(
        "omp.unroll_heuristic", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
