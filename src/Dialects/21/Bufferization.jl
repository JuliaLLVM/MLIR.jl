...
function dealloc(memrefs::Vector{Value}, conditions::Vector{Value}, retained::Vector{Value}; updatedConditions=nothing::Union{Nothing, Vector{IR.Type}}, location=Location())
    ...
    !isnothing(updatedConditions) && push!(op_ty_results, updatedConditions...)
...
"""
function to_buffer(tensor::Value; buffer::IR.Type, read_only=nothing, location=Location())
    ...
    create_operation(
        "bufferization.to_buffer", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function to_tensor(buffer::Value; result::IR.Type, restrict=nothing, writable=nothing, location=Location())
    op_ty_results = IR.Type[result, ]
