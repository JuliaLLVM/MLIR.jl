...
"""
function convert_layout(source::Value; result::IR.Type, srcMap, resMap, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[source, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("srcMap", srcMap), NamedAttribute("resMap", resMap), ]
    
    IR.create_operation(
        "xegpu.convert_layout", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
`load`
 ...
 ```
"""
function load(TensorDesc::Value, mask::Value; value::IR.Type, l1_hint=nothing, l2_hint=nothing, l3_hint=nothing, location=Location())
