...
"""
`EXT_ConstantCompositeReplicate`
...
```
"""
function EXT_ConstantCompositeReplicate(; replicated_constant::IR.Type, value, location=Location())
    op_ty_results = IR.Type[replicated_constant, ]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("value", value), ]
...
```
"""
function EXT_EmitMeshTasks(group_count_x::Value, group_count_y::Value, group_count_z::Value, payload=nothing::Union{Nothing, Value}; location=Location())
    op_ty_results = IR.Type[]
    operands = Value[group_count_x, group_count_y, group_count_z, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
```
"""
function EXT_SetMeshOutputs(vertex_count::Value, primitive_count::Value; location=Location())
    op_ty_results = IR.Type[]
    operands = Value[vertex_count, primitive_count, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
```
"""
function EXT_SpecConstantCompositeReplicate(; type, sym_name, constituent, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function EmitVertex(; location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function EndPrimitive(; location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function FunctionCall(arguments::Vector{Value}; return_value=nothing::Union{Nothing, IR.Type}, callee, arg_attrs=nothing, res_attrs=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[arguments..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("callee", callee), ]
    !isnothing(return_value) && push!(op_ty_results, return_value)
    !isnothing(arg_attrs) && push!(attributes, NamedAttribute("arg_attrs", arg_attrs))
    !isnothing(res_attrs) && push!(attributes, NamedAttribute("res_attrs", res_attrs))
...
```
"""
function GL_Acosh(operand::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GL_Asinh(operand::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GL_Atanh(operand::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GL_Cross(lhs::Value, rhs::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[lhs, rhs, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GL_Distance(p0::Value, p1::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[p0, p1, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(result) && push!(op_ty_results, result)
...
```
"""
function GL_Exp2(operand::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
computed per component.
"""
function GL_FindILsb(operand::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function GL_FindSMsb(operand::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GL_Fract(operand::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GL_Length(operand::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GL_Log2(operand::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GL_Normalize(operand::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GL_PackHalf2x16(operand::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GL_Reflect(lhs::Value, rhs::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[lhs, rhs, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GL_UnpackHalf2x16(operand::Value; result::IR.Type, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GroupNonUniformAllEqual(value::Value; result=nothing::Union{Nothing, IR.Type}, execution_scope, location=Location())
    op_ty_results = IR.Type[]
...
```
"""
function GroupNonUniformAll(predicate::Value; result=nothing::Union{Nothing, IR.Type}, execution_scope, location=Location())
    op_ty_results = IR.Type[]
...
```
"""
function GroupNonUniformAny(predicate::Value; result=nothing::Union{Nothing, IR.Type}, execution_scope, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[predicate, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GroupNonUniformBallotBitCount(value::Value; result::IR.Type, execution_scope, group_operation, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[value, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GroupNonUniformBallotFindLSB(value::Value; result::IR.Type, execution_scope, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[value, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GroupNonUniformBallotFindMSB(value::Value; result::IR.Type, execution_scope, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[value, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function GroupNonUniformRotateKHR(value::Value, delta::Value, cluster_size=nothing::Union{Nothing, Value}; result=nothing::Union{Nothing, IR.Type}, execution_scope, location=Location())
    op_ty_results = IR.Type[]
...
```
"""
function INTEL_ControlBarrierArrive(; execution_scope, memory_scope, memory_semantics, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function INTEL_ControlBarrierWait(; execution_scope, memory_scope, memory_semantics, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function ImageDrefGather(sampled_image::Value, coordinate::Value, dref::Value, operand_arguments::Vector{Value}; result::IR.Type, image_operands=nothing, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[sampled_image, coordinate, dref, operand_arguments..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
```
"""
function ImageFetch(image::Value, coordinate::Value, operand_arguments::Vector{Value}; result::IR.Type, image_operands=nothing, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[image, coordinate, operand_arguments..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(image_operands) && push!(attributes, NamedAttribute("image_operands", image_operands))
...
```
"""
function Image(sampled_image::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[sampled_image, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(result) && push!(op_ty_results, result)
...
```
"""
function ImageRead(image::Value, coordinate::Value, operand_arguments::Vector{Value}; result::IR.Type, image_operands=nothing, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[image, coordinate, operand_arguments..., ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function ImageSampleExplicitLod(sampled_image::Value, coordinate::Value, operand_arguments::Vector{Value}; result::IR.Type, image_operands, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[sampled_image, coordinate, operand_arguments..., ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function ImageSampleImplicitLod(sampled_image::Value, coordinate::Value, operand_arguments::Vector{Value}; result::IR.Type, image_operands=nothing, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[sampled_image, coordinate, operand_arguments..., ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function ImageSampleProjDrefImplicitLod(sampled_image::Value, coordinate::Value, dref::Value, operand_arguments::Vector{Value}; result=nothing::Union{Nothing, IR.Type}, image_operands=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[sampled_image, coordinate, dref, operand_arguments..., ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function ImageWrite(image::Value, coordinate::Value, texel::Value, operand_arguments::Vector{Value}; image_operands=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[image, coordinate, texel, operand_arguments..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(image_operands) && push!(attributes, NamedAttribute("image_operands", image_operands))
...
```
"""
function KHR_CooperativeMatrixLoad(pointer::Value, stride::Value; result::IR.Type, matrix_layout, memory_operand=nothing, alignment=nothing, location=Location())
    op_ty_results = IR.Type[result, ]
...
```
"""
function KHR_CooperativeMatrixStore(pointer::Value, object::Value, stride::Value; matrix_layout, memory_operand=nothing, alignment=nothing, location=Location())
    op_ty_results = IR.Type[]
...
"""
function mlir_loop(; results::Vector{IR.Type}, loop_control, body::Region, location=Location())
...
```
"""
function MatrixTimesVector(matrix::Value, vector::Value; result::IR.Type, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[matrix, vector, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
"""
function mlir_merge(operands::Vector{Value}; location=Location())
    op_ty_results = IR.Type[]
...
"""
function mlir_selection(; results::Vector{IR.Type}, selection_control, body::Region, location=Location())
    op_ty_results = IR.Type[results..., ]
    operands = Value[]
    owned_regions = Region[body, ]
    successors = Block[]
...
```
"""
function VectorTimesMatrix(vector::Value, matrix::Value; result::IR.Type, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[vector, matrix, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
