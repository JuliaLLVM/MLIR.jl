...
"""
function assume_alignment(memref::Value; result=nothing::Union{Nothing, IR.Type}, alignment, location=Location())
...
source/result layout map are the faster-varying ones.
"""
function expand_shape(src::Value, output_shape::Vector{Value}; result::IR.Type, reassociation, static_output_shape, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[src, output_shape..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("reassociation", reassociation), NamedAttribute("static_output_shape", static_output_shape), ]
