...
```
"""
function call(operands::Vector{Value}; result_0::Vector{IR.Type}, callee, arg_attrs=nothing, res_attrs=nothing, location=Location())
    op_ty_results = IR.Type[result_0..., ]
...
```
"""
function class(; sym_name, final_specifier=nothing, body::Region, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
...
```
"""
function field(; sym_name, type, attrs=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function file(; id, bodyRegion::Region, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
...
```
"""
function get_field(; result::IR.Type, field_name, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
...
function global_(; sym_name, type, initial_value=nothing, extern_specifier=nothing, static_specifier=nothing, const_specifier=nothing, location=Location())
    ...
    !isnothing(extern_specifier) && push!(attributes, NamedAttribute("extern_specifier", extern_specifier))
...
```
"""
function load(operand::Value; result::IR.Type, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[operand, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function subscript(value::Value, indices::Vector{Value}; result::IR.Type, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[value, indices..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
```
"""
function switch(arg::Value; cases, defaultRegion::Region, caseRegions::Vector{Region}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[arg, ]
...
```
"""
function verbatim(fmtArgs::Vector{Value}; value, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[fmtArgs..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("value", value), ]
