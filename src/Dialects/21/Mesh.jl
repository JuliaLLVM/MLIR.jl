...
"""
function get_sharding(source::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
...
"""
function shard_shape(dims_dynamic::Vector{Value}, sharding::Value, device_dynamic::Vector{Value}; result::Vector{IR.Type}, dims, device, location=Location())
    op_ty_results = IR.Type[result..., ]
    operands = Value[dims_dynamic..., sharding, device_dynamic..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("dims", dims), NamedAttribute("device", device), ]
    push!(attributes, operandsegmentsizes([length(dims_dynamic), 1, length(device_dynamic), ]))
