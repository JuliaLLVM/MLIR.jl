...
"""
function scaling_extf(in::Value, scale::Value; out::IR.Type, fastmath=nothing, location=Location())
    op_ty_results = IR.Type[out, ]
...
```
"""
function scaling_truncf(in::Value, scale::Value; out::IR.Type, roundingmode=nothing, fastmath=nothing, location=Location())
    op_ty_results = IR.Type[out, ]
    operands = Value[in, scale, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(roundingmode) && push!(attributes, NamedAttribute("roundingmode", roundingmode))
    !isnothing(fastmath) && push!(attributes, NamedAttribute("fastmath", fastmath))
    
    create_operation(
        "arith.scaling_truncf", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
When operating on vectors, casts elementwise.
"""
function truncf(in::Value; out::IR.Type, roundingmode=nothing, fastmath=nothing, location=Location())
    op_ty_results = IR.Type[out, ]
    operands = Value[in, ]
    owned_regions = Region[]
    successors = Block[]
