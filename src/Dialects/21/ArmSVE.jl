...
"""
function intr_bfmmla(acc::Value, src1::Value, src2::Value; res::IR.Type, location=Location())
...
function intr_dupq_lane(v::Value; res::IR.Type, lane, location=Location())
    op_ty_results = IR.Type[res, ]
    operands = Value[v, ]
    owned_regions = Region[]
    successors = Block[]
    ...
    create_operation(
        "arm_sve.intr.dupq_lane", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function dupq_lane(src::Value; dst::IR.Type, lane, location=Location())
...
function intr_psel(p1::Value, p2::Value, index::Value; res::IR.Type, location=Location())
    op_ty_results = IR.Type[res, ]
    operands = Value[p1, p2, index, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
"""
function psel(p1::Value, p2::Value, index::Value; result::IR.Type, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[p1, p2, index, ]
    owned_regions = Region[]
    successors = Block[]
...
function intr_usmmla(operand_0::Value, operand_1::Value, operand_2::Value; res::IR.Type, location=Location())
    op_ty_results = IR.Type[res, ]
    operands = Value[operand_0, operand_1, operand_2, ]
    owned_regions = Region[]
    successors = Block[]
...
https://developer.arm.com/documentation/100987/0000
"""
function usmmla(acc::Value, src1::Value, src2::Value; dst::IR.Type, location=Location())
    op_ty_results = IR.Type[dst, ]
    operands = Value[acc, src1, src2, ]
    owned_regions = Region[]
    successors = Block[]
...
function intr_whilelt(base::Value, n::Value; res::IR.Type, location=Location())
    op_ty_results = IR.Type[res, ]
...
function zip_x2(sourceV1::Value, sourceV2::Value; resultV1::IR.Type, resultV2::IR.Type, location=Location())
    op_ty_results = IR.Type[resultV1, resultV2, ]
...
function zip_x4(sourceV1::Value, sourceV2::Value, sourceV3::Value, sourceV4::Value; resultV1::IR.Type, resultV2::IR.Type, resultV3::IR.Type, resultV4::IR.Type, location=Location())
    op_ty_results = IR.Type[resultV1, resultV2, resultV3, resultV4, ]
