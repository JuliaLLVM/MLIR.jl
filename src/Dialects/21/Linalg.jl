...
"""
function winograd_filter_transform(filter::Value, output::Value; result::IR.Type, fmr, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[filter, output, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("fmr", fmr), ]
    
    create_operation(
        "linalg.winograd_filter_transform", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function winograd_input_transform(input::Value, output::Value; result::IR.Type, fmr, location=Location())
    op_ty_results = IR.Type[result, ]
    operands = Value[input, output, ]
    owned_regions = Region[]
    successors = Block[]
    ...
    create_operation(
        "linalg.winograd_input_transform", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function winograd_output_transform(value::Value, output::Value; result::IR.Type, fmr, location=Location())
    op_ty_results = IR.Type[result, ]
    ...
    create_operation(
        "linalg.winograd_output_transform", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
Numeric casting is performed on the operands to the inner multiply, promoting
    ...
    ```
"""
function batch_matmul(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, indexing_maps=nothing, cast=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
    attributes = NamedAttribute[]
    push!(attributes, operandsegmentsizes([length(inputs), length(outputs), ]))
    !isnothing(indexing_maps) && push!(attributes, NamedAttribute("indexing_maps", indexing_maps))
...
```
"""
function batch_reduce_matmul(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, indexing_maps=nothing, cast=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
TODO: Allow control over the combining/accumulating op and possibly the
      multiplication op.
"""
function contract(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, indexing_maps, cast=nothing, combiner::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[combiner, ]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("indexing_maps", indexing_maps), ]
    push!(attributes, operandsegmentsizes([length(inputs), length(outputs), ]))
    !isnothing(cast) && push!(attributes, NamedAttribute("cast", cast))
    
    create_operation(
        "linalg.contract", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
"""
function conv_2d_nchw_fchw_q(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, strides=nothing, dilations=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
    ...
    create_operation(
        "linalg.conv_2d_nchw_fchw_q", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
point offsets common to quantized operations.
"""
function conv_2d_ngchw_gfchw_q(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, strides=nothing, dilations=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
    ...
    create_operation(
        "linalg.conv_2d_ngchw_gfchw_q", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
them to the same data type as the accumulator/output.
"""
function conv_2d_nhwgc_gfhwc(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, strides=nothing, dilations=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
    ...
    create_operation(
        "linalg.conv_2d_nhwgc_gfhwc", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
point offsets common to quantized operations.
"""
function conv_2d_nhwgc_gfhwc_q(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, strides=nothing, dilations=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
    ...
    create_operation(
        "linalg.conv_2d_nhwgc_gfhwc_q", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
...
```
"""
function elementwise(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, kind, indexing_maps=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
No numeric casting is performed on the input operand.
"""
function erf(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
```
"""
function matmul(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, indexing_maps=nothing, cast=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
`linalg.generic` with different affine maps for the two operands.
"""
function min(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
`linalg.generic` with different affine maps for the two operands.
"""
function powf(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
No numeric casting is performed on the input operand.
"""
function reciprocal(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
No numeric casting is performed on the input operand.
"""
function round(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
No numeric casting is performed on the input operand.
"""
function rsqrt(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
`linalg.generic` with different affine maps for the two operands.
"""
function select(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
No numeric casting is performed on the input operand.
"""
function sqrt(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
No numeric casting is performed on the input operand.
"""
function square(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
...
No numeric casting is performed on the input operand.
"""
function tanh(inputs::Vector{Value}, outputs::Vector{Value}; result_tensors::Vector{IR.Type}, region::Region, location=Location())
    op_ty_results = IR.Type[result_tensors..., ]
    operands = Value[inputs..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
