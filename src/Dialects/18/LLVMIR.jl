...
```
"""
function func(; sym_name, sym_visibility=nothing, function_type, linkage=nothing, dso_local=nothing, CConv=nothing, comdat=nothing, personality=nothing, garbageCollector=nothing, passthrough=nothing, arg_attrs=nothing, res_attrs=nothing, function_entry_count=nothing, memory=nothing, visibility_=nothing, arm_streaming=nothing, arm_locally_streaming=nothing, arm_streaming_compatible=nothing, arm_new_za=nothing, arm_preserves_za=nothing, arm_shared_za=nothing, section=nothing, unnamed_addr=nothing, alignment=nothing, vscale_range=nothing, frame_pointer=nothing, target_cpu=nothing, target_features=nothing, body::Region, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[]
    owned_regions = Region[body, ]
    successors = Block[]
    ...
    !isnothing(arm_shared_za) && push!(attributes, NamedAttribute("arm_shared_za", arm_shared_za))
    !isnothing(section) && push!(attributes, NamedAttribute("section", section))
    !isnothing(unnamed_addr) && push!(attributes, NamedAttribute("unnamed_addr", unnamed_addr))
    !isnothing(alignment) && push!(attributes, NamedAttribute("alignment", alignment))
