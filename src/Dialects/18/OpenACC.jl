...
function attach(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, accPtr::IR.Type, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    ...
    push!(attributes, operandsegmentsizes([1, Int(!isnothing(varPtrPtr)), length(bounds), ]))
...
```
"""
function data(ifCond=nothing::Union{Nothing, Value}; async::Vector{Value}, waitDevnum=nothing::Union{Nothing, Value}, waitOperands::Vector{Value}, dataClauseOperands::Vector{Value}, asyncDeviceType=nothing, asyncOnly=nothing, waitOperandsSegments=nothing, waitOperandsDeviceType=nothing, waitOnly=nothing, defaultAttr=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[]
    ...
    push!(attributes, operandsegmentsizes([Int(!isnothing(ifCond)), length(async), Int(!isnothing(waitDevnum)), length(waitOperands), length(dataClauseOperands), ]))
    !isnothing(asyncDeviceType) && push!(attributes, NamedAttribute("asyncDeviceType", asyncDeviceType))
...
```
"""
function kernels(async::Vector{Value}, waitOperands::Vector{Value}, numGangs::Vector{Value}, numWorkers::Vector{Value}, vectorLength::Vector{Value}, ifCond=nothing::Union{Nothing, Value}; selfCond=nothing::Union{Nothing, Value}, dataClauseOperands::Vector{Value}, asyncDeviceType=nothing, asyncOnly=nothing, waitOperandsSegments=nothing, waitOperandsDeviceType=nothing, waitOnly=nothing, numGangsSegments=nothing, numGangsDeviceType=nothing, numWorkersDeviceType=nothing, vectorLengthDeviceType=nothing, selfAttr=nothing, defaultAttr=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[async..., waitOperands..., numGangs..., numWorkers..., vectorLength..., dataClauseOperands..., ]
    owned_regions = Region[region, ]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(ifCond) && push!(operands, ifCond)
    !isnothing(selfCond) && push!(operands, selfCond)
    push!(attributes, operandsegmentsizes([length(async), length(waitOperands), length(numGangs), length(numWorkers), length(vectorLength), Int(!isnothing(ifCond)), Int(!isnothing(selfCond)), length(dataClauseOperands), ]))
    !isnothing(asyncDeviceType) && push!(attributes, NamedAttribute("asyncDeviceType", asyncDeviceType))
    !isnothing(asyncOnly) && push!(attributes, NamedAttribute("asyncOnly", asyncOnly))
    !isnothing(waitOperandsSegments) && push!(attributes, NamedAttribute("waitOperandsSegments", waitOperandsSegments))
    !isnothing(waitOperandsDeviceType) && push!(attributes, NamedAttribute("waitOperandsDeviceType", waitOperandsDeviceType))
...
```
"""
function parallel(async::Vector{Value}, waitOperands::Vector{Value}, numGangs::Vector{Value}, numWorkers::Vector{Value}, vectorLength::Vector{Value}, ifCond=nothing::Union{Nothing, Value}; selfCond=nothing::Union{Nothing, Value}, reductionOperands::Vector{Value}, gangPrivateOperands::Vector{Value}, gangFirstPrivateOperands::Vector{Value}, dataClauseOperands::Vector{Value}, asyncDeviceType=nothing, asyncOnly=nothing, waitOperandsSegments=nothing, waitOperandsDeviceType=nothing, waitOnly=nothing, numGangsSegments=nothing, numGangsDeviceType=nothing, numWorkersDeviceType=nothing, vectorLengthDeviceType=nothing, selfAttr=nothing, reductionRecipes=nothing, privatizations=nothing, firstprivatizations=nothing, defaultAttr=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[async..., waitOperands..., numGangs..., numWorkers..., vectorLength..., reductionOperands..., gangPrivateOperands..., gangFirstPrivateOperands..., dataClauseOperands..., ]
    owned_regions = Region[region, ]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(ifCond) && push!(operands, ifCond)
    !isnothing(selfCond) && push!(operands, selfCond)
    push!(attributes, operandsegmentsizes([length(async), length(waitOperands), length(numGangs), length(numWorkers), length(vectorLength), Int(!isnothing(ifCond)), Int(!isnothing(selfCond)), length(reductionOperands), length(gangPrivateOperands), length(gangFirstPrivateOperands), length(dataClauseOperands), ]))
    !isnothing(asyncDeviceType) && push!(attributes, NamedAttribute("asyncDeviceType", asyncDeviceType))
    !isnothing(asyncOnly) && push!(attributes, NamedAttribute("asyncOnly", asyncOnly))
    !isnothing(waitOperandsSegments) && push!(attributes, NamedAttribute("waitOperandsSegments", waitOperandsSegments))
    !isnothing(waitOperandsDeviceType) && push!(attributes, NamedAttribute("waitOperandsDeviceType", waitOperandsDeviceType))
...
```
"""
function serial(async::Vector{Value}, waitOperands::Vector{Value}, ifCond=nothing::Union{Nothing, Value}; selfCond=nothing::Union{Nothing, Value}, reductionOperands::Vector{Value}, gangPrivateOperands::Vector{Value}, gangFirstPrivateOperands::Vector{Value}, dataClauseOperands::Vector{Value}, asyncDeviceType=nothing, asyncOnly=nothing, waitOperandsSegments=nothing, waitOperandsDeviceType=nothing, waitOnly=nothing, selfAttr=nothing, reductionRecipes=nothing, privatizations=nothing, firstprivatizations=nothing, defaultAttr=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[async..., waitOperands..., reductionOperands..., gangPrivateOperands..., gangFirstPrivateOperands..., dataClauseOperands..., ]
    owned_regions = Region[region, ]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(ifCond) && push!(operands, ifCond)
    !isnothing(selfCond) && push!(operands, selfCond)
    push!(attributes, operandsegmentsizes([length(async), length(waitOperands), Int(!isnothing(ifCond)), Int(!isnothing(selfCond)), length(reductionOperands), length(gangPrivateOperands), length(gangFirstPrivateOperands), length(dataClauseOperands), ]))
    !isnothing(asyncDeviceType) && push!(attributes, NamedAttribute("asyncDeviceType", asyncDeviceType))
    !isnothing(asyncOnly) && push!(attributes, NamedAttribute("asyncOnly", asyncOnly))
    !isnothing(waitOperandsSegments) && push!(attributes, NamedAttribute("waitOperandsSegments", waitOperandsSegments))
    !isnothing(waitOperandsDeviceType) && push!(attributes, NamedAttribute("waitOperandsDeviceType", waitOperandsDeviceType))
...
```
"""
function update(ifCond=nothing::Union{Nothing, Value}; asyncOperand=nothing::Union{Nothing, Value}, waitDevnum=nothing::Union{Nothing, Value}, waitOperands::Vector{Value}, dataClauseOperands::Vector{Value}, async=nothing, wait=nothing, device_types=nothing, ifPresent=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[waitOperands..., dataClauseOperands..., ]
    owned_regions = Region[]
    successors = Block[]
