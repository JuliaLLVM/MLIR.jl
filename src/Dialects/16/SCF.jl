...
```
"""
function foreach_thread(num_threads::Vector{Value}, outputs::Vector{Value}; results::Vector{IR.Type}, mapping=nothing, region::Region, location=Location())
    op_ty_results = IR.Type[results..., ]
    operands = Value[num_threads..., outputs..., ]
    owned_regions = Region[region, ]
    successors = Block[]
    attributes = NamedAttribute[]
