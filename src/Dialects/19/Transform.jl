...
"""
function structured_pad(target::Value, pad_to_multiple_of::Vector{Value}; padded::IR.Type, pad::IR.Type, copy::IR.Type, padding_values=nothing, padding_dimensions=nothing, static_pad_to_multiple_of=nothing, pack_paddings=nothing, transpose_paddings=nothing, copy_back_op=nothing, location=Location())
...
"""
function structured_split(target::Value, dynamic_chunk_sizes=nothing::Union{Nothing, Value}; first::IR.Type, second::IR.Type, dimension, static_chunk_sizes, multiway=nothing, location=Location())
    op_ty_results = IR.Type[first, second, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function structured_tile_reduction_using_for(target::Value; fill_op::Vector{IR.Type}, split_linalg_op::IR.Type, combining_linalg_op::IR.Type, for_op::IR.Type, tile_sizes=nothing, location=Location())
    op_ty_results = IR.Type[fill_op..., split_linalg_op, combining_linalg_op, for_op, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
