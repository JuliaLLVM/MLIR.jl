...
"""
function attach(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, asyncOperands::Vector{Value}, accPtr::IR.Type, asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[accPtr, ]
    operands = Value[varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
using the activity queues on these devices as defined by the async values.
"""
function cache(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, asyncOperands::Vector{Value}, accPtr::IR.Type, asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[accPtr, ]
    operands = Value[varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
...
using the activity queues on these devices as defined by the async values.
"""
function copyin(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, asyncOperands::Vector{Value}, accPtr::IR.Type, asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[accPtr, ]
    operands = Value[varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
...
- `varPtr`: The address of variable to copy back to.
    ...
    using the activity queues on these devices as defined by the async values.
"""
function copyout(accPtr::Value, varPtr::Value, bounds::Vector{Value}, asyncOperands::Vector{Value}; asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[accPtr, varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
using the activity queues on these devices as defined by the async values.
"""
function create(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, asyncOperands::Vector{Value}, accPtr::IR.Type, asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[accPtr, ]
    operands = Value[varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
...
using the activity queues on these devices as defined by the async values.
"""
function declare_device_resident(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, asyncOperands::Vector{Value}, accPtr::IR.Type, asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[accPtr, ]
    operands = Value[varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
...
using the activity queues on these devices as defined by the async values.
"""
function declare_link(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, asyncOperands::Vector{Value}, accPtr::IR.Type, asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[accPtr, ]
    operands = Value[varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
...
using the activity queues on these devices as defined by the async values.
"""
function delete(accPtr::Value, bounds::Vector{Value}, asyncOperands::Vector{Value}; asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[accPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
using the activity queues on these devices as defined by the async values.
"""
function detach(accPtr::Value, bounds::Vector{Value}, asyncOperands::Vector{Value}; asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[accPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
...
using the activity queues on these devices as defined by the async values.
"""
function deviceptr(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, asyncOperands::Vector{Value}, accPtr::IR.Type, asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[accPtr, ]
    operands = Value[varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
...
using the activity queues on these devices as defined by the async values.
"""
function firstprivate(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, asyncOperands::Vector{Value}, accPtr::IR.Type, asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[accPtr, ]
    operands = Value[varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
...
\\
    ...
    using the activity queues on these devices as defined by the async values.
"""
function getdeviceptr(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, asyncOperands::Vector{Value}, accPtr::IR.Type, asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[accPtr, ]
    operands = Value[varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
...
using the activity queues on these devices as defined by the async values.
"""
function nocreate(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, asyncOperands::Vector{Value}, accPtr::IR.Type, asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[accPtr, ]
    operands = Value[varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
...
function parallel(asyncOperands::Vector{Value}, waitOperands::Vector{Value}, numGangs::Vector{Value}, numWorkers::Vector{Value}, vectorLength::Vector{Value}, ifCond=nothing::Union{Nothing, Value}; selfCond=nothing::Union{Nothing, Value}, reductionOperands::Vector{Value}, gangPrivateOperands::Vector{Value}, gangFirstPrivateOperands::Vector{Value}, dataClauseOperands::Vector{Value}, asyncOperandsDeviceType=nothing, asyncOnly=nothing, waitOperandsSegments=nothing, waitOperandsDeviceType=nothing, hasWaitDevnum=nothing, waitOnly=nothing, numGangsSegments=nothing, numGangsDeviceType=nothing, numWorkersDeviceType=nothing, vectorLengthDeviceType=nothing, selfAttr=nothing, reductionRecipes=nothing, privatizations=nothing, firstprivatizations=nothing, defaultAttr=nothing, combined=nothing, region::Region, location=Location())
    ...
    !isnothing(ifCond) && push!(operands, ifCond)
    !isnothing(selfCond) && push!(operands, selfCond)
    push!(attributes, operandsegmentsizes([length(asyncOperands), length(waitOperands), length(numGangs), length(numWorkers), length(vectorLength), Int(!isnothing(ifCond)), Int(!isnothing(selfCond)), length(reductionOperands), length(gangPrivateOperands), length(gangFirstPrivateOperands), length(dataClauseOperands), ]))
    !isnothing(asyncOperandsDeviceType) && push!(attributes, NamedAttribute("asyncOperandsDeviceType", asyncOperandsDeviceType))
    !isnothing(asyncOnly) && push!(attributes, NamedAttribute("asyncOnly", asyncOnly))
    !isnothing(waitOperandsSegments) && push!(attributes, NamedAttribute("waitOperandsSegments", waitOperandsSegments))
    !isnothing(waitOperandsDeviceType) && push!(attributes, NamedAttribute("waitOperandsDeviceType", waitOperandsDeviceType))
...
using the activity queues on these devices as defined by the async values.
"""
function present(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, asyncOperands::Vector{Value}, accPtr::IR.Type, asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[accPtr, ]
    operands = Value[varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
...
using the activity queues on these devices as defined by the async values.
"""
function private(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, asyncOperands::Vector{Value}, accPtr::IR.Type, asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[accPtr, ]
    operands = Value[varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
...
using the activity queues on these devices as defined by the async values.
"""
function reduction(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, asyncOperands::Vector{Value}, accPtr::IR.Type, asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[accPtr, ]
    operands = Value[varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
...
function serial(asyncOperands::Vector{Value}, waitOperands::Vector{Value}, ifCond=nothing::Union{Nothing, Value}; selfCond=nothing::Union{Nothing, Value}, reductionOperands::Vector{Value}, gangPrivateOperands::Vector{Value}, gangFirstPrivateOperands::Vector{Value}, dataClauseOperands::Vector{Value}, asyncOperandsDeviceType=nothing, asyncOnly=nothing, waitOperandsSegments=nothing, waitOperandsDeviceType=nothing, hasWaitDevnum=nothing, waitOnly=nothing, selfAttr=nothing, reductionRecipes=nothing, privatizations=nothing, firstprivatizations=nothing, defaultAttr=nothing, combined=nothing, region::Region, location=Location())
    ...
    !isnothing(ifCond) && push!(operands, ifCond)
    !isnothing(selfCond) && push!(operands, selfCond)
    push!(attributes, operandsegmentsizes([length(asyncOperands), length(waitOperands), Int(!isnothing(ifCond)), Int(!isnothing(selfCond)), length(reductionOperands), length(gangPrivateOperands), length(gangFirstPrivateOperands), length(dataClauseOperands), ]))
    !isnothing(asyncOperandsDeviceType) && push!(attributes, NamedAttribute("asyncOperandsDeviceType", asyncOperandsDeviceType))
    !isnothing(asyncOnly) && push!(attributes, NamedAttribute("asyncOnly", asyncOnly))
    !isnothing(waitOperandsSegments) && push!(attributes, NamedAttribute("waitOperandsSegments", waitOperandsSegments))
    !isnothing(waitOperandsDeviceType) && push!(attributes, NamedAttribute("waitOperandsDeviceType", waitOperandsDeviceType))
...
using the activity queues on these devices as defined by the async values.
"""
function update_device(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, asyncOperands::Vector{Value}, accPtr::IR.Type, asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[accPtr, ]
    operands = Value[varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
...
- `varPtr`: The address of variable to copy back to.
    ...
    using the activity queues on these devices as defined by the async values.
"""
function update_host(accPtr::Value, varPtr::Value, bounds::Vector{Value}, asyncOperands::Vector{Value}; asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[accPtr, varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
...
using the activity queues on these devices as defined by the async values.
"""
function use_device(varPtr::Value, varPtrPtr=nothing::Union{Nothing, Value}; bounds::Vector{Value}, asyncOperands::Vector{Value}, accPtr::IR.Type, asyncOperandsDeviceType=nothing, asyncOnly=nothing, dataClause=nothing, structured=nothing, implicit=nothing, name=nothing, location=Location())
    op_ty_results = IR.Type[accPtr, ]
    operands = Value[varPtr, bounds..., asyncOperands..., ]
    owned_regions = Region[]
    successors = Block[]
