...
```
"""
function leading_term(input::Value; degree::IR.Type, coefficient::IR.Type, location=Location())
    op_ty_results = IR.Type[degree, coefficient, ]
    operands = Value[input, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
"""
function monic_monomial_mul(input::Value, monomialDegree::Value; output=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[input, monomialDegree, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(output) && push!(op_ty_results, output)
...
function monomial(coefficient::Value, degree::Value; output::IR.Type, location=Location())
    op_ty_results = IR.Type[output, ]
    operands = Value[coefficient, degree, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
```
"""
function mul_scalar(polynomial::Value, scalar::Value; output=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[polynomial, scalar, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(output) && push!(op_ty_results, output)
