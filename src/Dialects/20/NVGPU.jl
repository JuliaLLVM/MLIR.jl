...
```
"""
function device_async_copy(dst::Value, dstIndices::Vector{Value}, src::Value, srcIndices::Vector{Value}, srcElements=nothing::Union{Nothing, Value}; asyncToken=nothing::Union{Nothing, IR.Type}, dstElements, bypassL1=nothing, location=Location())
...
%0 = nvgpu.device_async_create_group
  ```
"""
function device_async_create_group(inputTokens::Vector{Value}; asyncToken=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
...
```
"""
function mbarrier_arrive_nocomplete(barriers::Value, mbarId::Value, count::Value; token=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[barriers, mbarId, count, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(token) && push!(op_ty_results, token)
...
```
"""
function mbarrier_arrive(barriers::Value, mbarId::Value; token=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
...
function mbarrier_create(; barriers::IR.Type, location=Location())
    op_ty_results = IR.Type[barriers, ]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
...
```
"""
function mbarrier_test_wait(barriers::Value, token::Value, mbarId::Value; waitComplete=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[barriers, token, mbarId, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(waitComplete) && push!(op_ty_results, waitComplete)
...
"""
function rcp(in::Value; out=nothing::Union{Nothing, IR.Type}, rounding=nothing, ftz=nothing, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[in, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(out) && push!(op_ty_results, out)
...
function tma_async_load(dst::Value, barriers::Value, tensorMapDescriptor::Value, coordinates::Vector{Value}, mbarId::Value, multicastMask=nothing::Union{Nothing, Value}; predicate=nothing::Union{Nothing, Value}, location=Location())
    ...
    !isnothing(predicate) && push!(operands, predicate)
    push!(attributes, operandsegmentsizes([1, 1, 1, length(coordinates), 1, Int(!isnothing(multicastMask)), Int(!isnothing(predicate)), ]))
...
function tma_create_descriptor(tensor::Value, boxDimensions::Vector{Value}; tensorMap::IR.Type, location=Location())
    op_ty_results = IR.Type[tensorMap, ]
...
function warpgroup_generate_descriptor(tensor::Value, tensorMap::Value; descriptor::IR.Type, location=Location())
    op_ty_results = IR.Type[descriptor, ]
