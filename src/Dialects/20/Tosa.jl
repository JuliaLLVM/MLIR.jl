...
"""
function clamp(input::Value; output::IR.Type, min_int, max_int, min_fp, max_fp, nan_mode=nothing, location=Location())
    op_ty_results = IR.Type[output, ]
    operands = Value[input, ]
    owned_regions = Region[]
    successors = Block[]
...
```
"""
function const_shape(; output::IR.Type, value, location=Location())
    op_ty_results = IR.Type[output, ]
    operands = Value[]
    owned_regions = Region[]
    successors = Block[]
...
tensor.
"""
function conv2d(input::Value, weight::Value, bias::Value; output::IR.Type, pad, stride, dilation, acc_type, quantization_info=nothing, local_bound=nothing, location=Location())
    op_ty_results = IR.Type[output, ]
...
"""
function conv3d(input::Value, weight::Value, bias::Value; output::IR.Type, pad, stride, dilation, acc_type, quantization_info=nothing, local_bound=nothing, location=Location())
    op_ty_results = IR.Type[output, ]
    operands = Value[input, weight, bias, ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function depthwise_conv2d(input::Value, weight::Value, bias::Value; output::IR.Type, pad, stride, dilation, acc_type, quantization_info=nothing, local_bound=nothing, location=Location())
    op_ty_results = IR.Type[output, ]
    operands = Value[input, weight, bias, ]
    owned_regions = Region[]
    successors = Block[]
...
"""
function mul(input1::Value, input2::Value, shift=nothing::Union{Nothing, Value}; output::IR.Type, location=Location())
    op_ty_results = IR.Type[output, ]
    operands = Value[input1, input2, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[]
    !isnothing(shift) && push!(operands, shift)
...
"""
function rescale(input::Value; output::IR.Type, input_zp, output_zp, multiplier, shift, scale32, double_round, per_channel, input_unsigned=nothing, output_unsigned=nothing, location=Location())
    op_ty_results = IR.Type[output, ]
    operands = Value[input, ]
    owned_regions = Region[]
    successors = Block[]
    ...
    !isnothing(input_unsigned) && push!(attributes, NamedAttribute("input_unsigned", input_unsigned))
    !isnothing(output_unsigned) && push!(attributes, NamedAttribute("output_unsigned", output_unsigned))
...
"""
function slice(input1::Value; output::IR.Type, start, size, location=Location())
    op_ty_results = IR.Type[output, ]
    operands = Value[input1, ]
    owned_regions = Region[]
    successors = Block[]
...
weights tensor.
"""
function transpose_conv2d(input::Value, filter::Value, bias::Value; output::IR.Type, out_pad, stride, out_shape, acc_type, quantization_info=nothing, local_bound=nothing, location=Location())
    op_ty_results = IR.Type[output, ]
    operands = Value[input, filter, bias, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("out_pad", out_pad), NamedAttribute("stride", stride), NamedAttribute("out_shape", out_shape), NamedAttribute("acc_type", acc_type), ]
    !isnothing(quantization_info) && push!(attributes, NamedAttribute("quantization_info", quantization_info))
    !isnothing(local_bound) && push!(attributes, NamedAttribute("local_bound", local_bound))
