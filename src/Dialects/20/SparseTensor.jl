...
```
"""
function extract_iteration_space(tensor::Value, parentIter=nothing::Union{Nothing, Value}; extractedSpace=nothing::Union{Nothing, IR.Type}, loLvl, hiLvl, location=Location())
    op_ty_results = IR.Type[]
    operands = Value[tensor, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("loLvl", loLvl), NamedAttribute("hiLvl", hiLvl), ]
    !isnothing(parentIter) && push!(operands, parentIter)
    !isnothing(extractedSpace) && push!(op_ty_results, extractedSpace)
    
    create_operation(
        "sparse_tensor.extract_iteration_space", location;
...
```
"""
function extract_value(tensor::Value, iterator::Value; result=nothing::Union{Nothing, IR.Type}, location=Location())
    op_ty_results = IR.Type[]
    ...
    create_operation(
        "sparse_tensor.extract_value", location;
        operands, owned_regions, successors, attributes,
        results=(length(op_ty_results) == 0 ? nothing : op_ty_results),
        result_inference=(length(op_ty_results) == 0 ? true : false)
    )
...
```
"""
function iterate(iterSpace::Value, initArgs::Vector{Value}; results::Vector{IR.Type}, crdUsedLvls, region::Region, location=Location())
    ...
    create_operation(
        "sparse_tensor.iterate", location;
        operands, owned_regions, successors, attributes,
...
```
"""
function coiterate(iterSpaces::Vector{Value}, initArgs::Vector{Value}; results::Vector{IR.Type}, crdUsedLvls, cases, caseRegions::Vector{Region}, location=Location())
    op_ty_results = IR.Type[results..., ]
    operands = Value[iterSpaces..., initArgs..., ]
    owned_regions = Region[caseRegions..., ]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("crdUsedLvls", crdUsedLvls), NamedAttribute("cases", cases), ]
    push!(attributes, operandsegmentsizes([length(iterSpaces), length(initArgs), ]))
    
    create_operation(
        "sparse_tensor.coiterate", location;
        operands, owned_regions, successors, attributes,
        results=op_ty_results,
        result_inference=false
    )
