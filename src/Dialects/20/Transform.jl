...
"""
function structured_pad(target::Value, pad_to_multiple_of::Vector{Value}; padded::IR.Type, pad::IR.Type, copy::IR.Type, padding_values=nothing, padding_dimensions=nothing, static_pad_to_multiple_of=nothing, nofold_flags=nothing, transpose_paddings=nothing, copy_back_op=nothing, location=Location())
...
```
"""
function structured_tile_reduction_using_for(target::Value; fill_op::Vector{IR.Type}, split_op::IR.Type, combining_op::IR.Type, for_op::IR.Type, tile_sizes=nothing, location=Location())
...
```
"""
function structured_tile_reduction_using_forall(target::Value; fill_op::Vector{IR.Type}, split_linalg_op::IR.Type, combining_linalg_op::IR.Type, forall_op::IR.Type, num_threads=nothing, tile_sizes=nothing, mapping=nothing, location=Location())
...
"""
function structured_winograd_conv2d(target::Value; transformed::IR.Type, m, r, location=Location())
    op_ty_results = IR.Type[transformed, ]
    operands = Value[target, ]
    owned_regions = Region[]
    successors = Block[]
    attributes = NamedAttribute[NamedAttribute("m", m), NamedAttribute("r", r), ]
...
"""
function include_(operands::Vector{Value}; results::Vector{IR.Type}, target, failure_propagation_mode, location=Location())
    op_ty_results = IR.Type[results..., ]
