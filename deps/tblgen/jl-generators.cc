...
bool emitOpTableDefs(llvm::raw_ostream &os, const llvm::RecordKeeper &recordKeeper, bool disableModuleWrap, bool isExternal, std::optional<std::string> dialectName)
{
  ...
  std::string modulename = dialectName.has_value() ? dialectName.value() : getDialectName(opdefs);

  // os << "# This file was auto-generated by mlir-tblgen from the following source files:\n";
  // for (const auto *def : opdefs) {
  //   mlir::tblgen::Operator op(*def);
  //   for (const auto &loc : op.getLoc()) {
  //     unsigned bufferID = llvm::SrcMgr.FindBufferContainingLoc(locs.front());
  //     os << "#   " << loc.getFilename() << "\n";
  //   }
  // }
  ...
  // emit imports
  os << "import ...IR: IR, NamedAttribute, Value, Location, Block, Region, Attribute, context, IndexType\n";
  os << "import ..Dialects: operandsegmentsizes, resultsegmentsizes\n";
...
function {0}({1}location=Location())
  ...
  for (const auto *def : opdefs) {
    ...
    os << llvm::formatv(functiontemplate, functionname, arguments,
                                    functionbody, description);
  }

  if (!disableModuleWrap)
    os << "end # module " << modulename << "\n";
